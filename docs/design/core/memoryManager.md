...
6. 确认目录提交后再删除 B 旧副本与暂存/journal：清理是独立阶段，必须在包含新目录的整串写入成功之后才执行（否则写失败叠加 global reset 会留下"目录指向已清空页"的悬空引用）。

在第 3～5 步，正式目录可能仍指向旧 Segment，因此恢复逻辑必须优先解释 journal。只保留副本而恢复时不查 journal，仍然会造成错误读取。

恢复 copy/verify/switch 时，申请入口及 begin 必须先根据 journal 建立冻结，再装载数据或执行业务 schema 升级；页不可见不能覆盖冻结语义。目录切换后从目标后端恢复数据，再处理延迟申请的数据版本升级。
...
迁移中止清理目标页和目录切换后的源页清理，都必须同时满足：页仍在管理范围、没有任何目录引用、信封 owner/代次/数据版本与清理凭证一致。源页的 fromGeneration 随 journal 保存，目标页使用本次迁移代次；旧未切换记录可从源目录补齐凭证，旧 cleanup 缺失凭证时保留页面并诊断。页不可见时不写；switch 前再次验证目标，防止 verify 后页面被替换。
...
剩余验证项（后续独立任务，登记在此避免只在变更记录里出现）：verify 阶段读取异常的故障注入、主 Memory 暂存容量的专项计量。
