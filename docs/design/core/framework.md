...
级联挂起只由提供者“不在启用集合中”触发。`optional` 的使用者不随提供者挂起，也不随提供者重启而重新 setup；同一批命令中以同一 id 替换提供者时，提供者释放后重新 setup，而它仍在启用集合中，`requires` 的使用者同样不会重新 setup。服务对象属于提供者的某次激活，因此使用者必须在使用时调用 `services.get`，不在 setup 中保存服务对象；提供者不可用时 `services.get` 抛错，由使用者处理。

待决设计事项：提供者的激活实例变化时（可选依赖的提供者重启、同批替换提供者），是否级联重新 setup 使用者。级联可以让使用者安全地缓存服务，代价是提供者的每次重启都会释放并重建这些使用者的资源。
