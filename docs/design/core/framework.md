...
`requires` 的提供者停用、熔断或卸载时，使用者挂起。按逆序运行激活实例清理函数并移除服务。重新启用后创建新激活实例，健康记录在同一 Framework 实例中保留。

级联挂起由两件事触发：提供者不在启用集合中，或注册表中的提供者实例被替换（同一批命令里先 `unregister` 再 `register` 同一个 id）。两者都会结束提供者的激活，因此 `requires` 的使用者一并释放并在之后重新 setup，使“使用者的激活包含在提供者的激活之内”这条语义成立。

`optional` 的使用者不随提供者挂起，也不随提供者重启而重新 setup：可选依赖本就允许提供者缺席。服务对象属于提供者的某次激活，因此所有使用者都必须在使用时调用 `services.get`，不在 setup 中保存服务对象；提供者不可用时 `services.get` 抛错，由使用者处理。

待决设计事项：可选依赖的提供者重启时，是否级联重新 setup 使用者。级联可以让使用者安全地缓存服务，代价是提供者的每次重启都会释放并重建这些本可容忍缺席的使用者。
