...
- 记录只用路径写入：MemoryManager 在修改前预检路径与值，非法的 owner 或任务 id 只让该条记录写入失败，不阻断整串写盘。存储不可用时跳过记录并告警一次，其余功能不受影响。上一 global 留下、本 global 在 `retainTicks` 内无人认领的记录会被清理；本 global 从未使用任务时分区保持原样。
