...
- 前缀使用加粗着色，正文保持原样：Screeps 控制台以 HTML 渲染，颜色只用于快速区分来源与严重程度。
- 一次输出只做一次前缀拼接与一次端口写入，不保留历史、不做缓冲；日志文本保持单行，避免控制台把一段内容拆成多条消息。

## 5. 输出端口与邮件策略

- `LogOutput` 定义两个通道：`write`（控制台）与 `notify`（邮件）。默认实现分别是 `console.log` 与 `Game.notify(line, notifyInterval)`；装配方可以整条通道替换，用于测试收集、静默或转存。
- 邮件策略是装配级开关 `notify: 'off' | 'error'`，默认 `off`；作用域的 `notify` 覆盖装配策略，`undefined` 表示跟随。只有 `error` 等级会考虑邮件，其余等级即使开启该作用域开关也不发送。
