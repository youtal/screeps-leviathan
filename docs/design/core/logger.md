...
交付状态：等级、作用域、格式化、惰性日志内容、双通道输出与 Runtime 单一组合根装配已交付；日志限流/采样与结构化字段未交付。
...
- 日志内容 `LogContent` 可以是字符串，也可以是返回字符串的惰性回调。回调只在等级开启时调用一次，得到的文本同时用于控制台与邮件；等级关闭时回调及其中的计算都不执行，代价只剩调用方创建闭包。它与 `isEnabled` 解决同一问题：单条消息用回调更简洁，同一判断下要输出多条或做额外工作时用 `isEnabled`。不采用可变参数：实参仍会在调用前求值，昂贵的参数计算省不掉，还需要定义对象格式化规则。
...
- 惰性回调抛错时丢弃该条日志，与输出端口失败同样静默降级，不向调用方传播，也不再记录失败以免形成递归；JavaScript 调用方返回非字符串时按 `String()` 转换。
