 ...
 * 文件摘要
 *
 * 模块角色：项目级默认配置表，为日志、性能统计等能力提供初始设置。
 *
 * 主要功能：导出日志等级开关、Profiler 默认开关和分组事件的 TTL 常量。
 *
 * 实现过程：用具名常量提供配置，Logger 和 Runtime 在创建实例时读取对应默认值，
 * 调用方可通过各自工厂的选项覆盖；TTL 常量本身不会启动任何过期清理。
 *
 * 技术要点：这里不读取 Game 或存储，也不动态刷新配置。日志默认表使用 warning 字段，
 * Logger 实现将它对应到 warn 方法；Profiler 默认关闭。
 */
