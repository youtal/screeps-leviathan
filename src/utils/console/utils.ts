...
export const replaceHtml = function(
  ...
  return Object.keys(replaceContent).reduce((html, nextKey) => {
    return html.replace(
      new RegExp(`{${nextKey}}`, "g"),
      replaceContent[nextKey]
    );
  }, html);
};
 ...
 * @param bolder 是否加粗
 */
export const colorful = function(
  content: string | number,
  colorName: Color | null = null,
  bold = false
): string {
...
export const green = (content: string | number, bold?: boolean) =>
  colorful(content, Color.Green, bold);
export const red = (content: string | number, bold?: boolean) =>
  colorful(content, Color.Red, bold);
export const yellow = (content: string | number, bold?: boolean) =>
  colorful(content, Color.Yellow, bold);
export const blue = (content: string | number, bold?: boolean) =>
  colorful(content, Color.Blue, bold);
...
export const createLog = (prefix: string) => ({
  normal: (content: string, notify = false) =>
    log(content, prefix, null, notify),
  success: (content: string, notify = false) =>
    log(content, prefix, Color.Green, notify),
  warning: (content: string, notify = false) =>
    log(content, prefix, Color.Yellow, notify),
  error: (content: string, notify = true) =>
    log(content, prefix, Color.Red, notify),
});
