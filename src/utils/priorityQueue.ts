 ...
 *
 * 模块位置：src/utils 下的通用数据结构，与 console 工具并列，由 src/utils/index.ts
 * 统一导出。goto 模块的跨房 A* 与房内 Dijkstra 依赖它以 O(log n) 取得当前最优候选，
 * 避免每轮对候选集合重新排序。
 *
 * 主要输入 / 输出：构造参数是初始元素数组与比较器；对外能力为 push 入队、pop 弹出
 * 最高优先级元素、peek 只读查看、clear 清空，以及 size/isEmpty 只读属性。
 * 泛型 T 由调用方决定，队列本身不解释元素内容。
 *
 * 状态与副作用：唯一状态是实例内部的数组，不访问 Game 与 Memory，不产生全局副作用。
 * 实例不可序列化，因此只能放在 heap（模块闭包或函数局部）；global reset 后需重新构造，
 * 队列也不会跨 tick 保留排序状态。
 *
 * 复杂度：建堆 O(n)，push/pop 均为 O(log n)，peek/size/isEmpty 为 O(1)。
 */
